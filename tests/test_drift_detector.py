...
def test_monitor_none_until_window_full() -> None:
    ...
    snapshot = monitor.update("dog_bark", 0.7)
...
def test_monitor_confidence_ks_when_quantiles_present() -> None:
    ...
    monitor = DriftMonitor(window_size=2, reference=reference)
    assert monitor.update("dog_bark", 0.2) is None
    snapshot = monitor.update("dog_bark", 0.1)
    assert snapshot is not None
...
def test_feature_psi_near_zero_on_same_draw() -> None:
    ...
    scored = score_features(histograms, {"mfcc_0_mean": values})
...
def test_cli_build_reference_from_processed(tmp_path: Path) -> None:
    ...
    dest = tmp_path / "drift_reference.json"
    dump_reference(dest, reference)
    loaded = DriftReference.from_dict(json.loads(dest.read_text()))
...
def test_cli_score_predictions_jsonl(tmp_path: Path) -> None:
    ...
    scored = score_prediction_file(log_path, reference)
...
def test_cli_writes_missing_prediction_jsonl(tmp_path: Path) -> None:
    ...
    scored = score_prediction_file(path, official_fold10_reference())
...
def test_cli_score_features(tmp_path: Path) -> None:
    ...
    histograms = build_feature_histograms({"mfcc_0_mean": ref_values}, n_bins=8)
    reference = with_feature_histograms(official_fold10_reference(), histograms)
    ref_path = tmp_path / "ref.json"
    dump_reference(ref_path, reference)
    frame = pd.DataFrame({"mfcc_0_mean": live_values, "fold": [10] * 80})
    parquet = tmp_path / "live.parquet"
    frame.to_parquet(parquet)
    scored = score_feature_file(parquet, reference, n_mfcc=13)
...
def test_cli_score_features_accepts_with_features(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    processed = tmp_path / "processed"
    processed.mkdir()
    parquet = processed / "tabular.parquet"
    frame.to_parquet(parquet)
    ...
    config_path.write_text(
        "\n".join(
            [
                ...
                "spark:",
                f"  local_processed_dir: {processed}",
                "  n_mfcc: 13",
                "preprocessing:",
                f"  local_interim_dir: {tmp_path / 'interim'}",
                "monitoring:",
                "  drift:",
                f"    reference_path: {tmp_path / 'drift_reference.json'}",
                "    n_feature_bins: 4",
            ]
        )
        + "\n"
    ...
    loaded = DriftReference.from_dict(
