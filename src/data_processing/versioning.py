...
def resolve_dataset_repo_id(config_path: Path = CONFIG_PATH) -> tuple[str, str]:
    ...
    cfg: AppConfig = load_full_config(config_path)
    dataset = cfg["dataset"]
...
def config_enabled(
    ...
    *,
    ...
    section_cfg: dict[str, object] = cast(
        dict[str, object], load_full_config(config_path).get(section) or {}
    )
    return bool(section_cfg.get(key, default))
...
def resolve_model_repo_id(config_path: Path = CONFIG_PATH) -> tuple[str, str]:
    ...
    vcfg: VersioningConfig = load_versioning_config(config_path)
...
def push_dataset_tree(
    ...
    *,
    ...
    resolved_repo_id: str = repo_id or default_repo_id
    resolved_repo_type: str = repo_type or default_repo_type
    resolved_path: str = path_in_repo or vcfg.get("path_in_repo", "interim")
    local_path: Path = Path(local_dir)
    if not local_path.exists():
        raise FileNotFoundError(local_path)

    _ensure_repo(resolved_repo_id, resolved_repo_type, private)
    ...
    result = upload_folder(
        ...
        commit_message=f"Update {resolved_path} from local {local_path.name}",
    )
    logger.info(
        "Pushed %s -> %s:%s (%s)", local_path, resolved_repo_id, resolved_path, result
    )
    return str(result)
...
def push_model_tree(
    ...
    *,
    ...
    result = upload_folder(
        ...
        or f"Update {path_in_repo} from local {local_path.name}",
    )
    logger.info(
        "Pushed %s -> %s:%s (%s)", local_path, resolved_repo_id, path_in_repo, result
    )
    return str(result)
...
def push_winner_artifacts(
    ...
    *,
    ...
    models_path: Path = Path(models_dir)
    winner_json: Path = models_path / "winner.json"
    winner_dir: Path = models_path / "winner"
...
def pull_dataset_tree(
    ...
    *,
    ...
    logger.info(
        "Pulled %s@%s into %s", resolved_repo_id, revision or "main", local_path
    )
    return local_path
...
def pull_winner_artifacts(
    ...
    *,
    ...
    logger.info(
        "Pulled winner artifacts from %s into %s", resolved_repo_id, models_path
    )
    return models_path
