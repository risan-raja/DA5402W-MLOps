...
def ensure_resnet18_weights() -> Path:
    ...
    try:
        with urllib.request.urlopen(url, context=ctx, timeout=120) as resp, open(tmp, "wb") as out:
            while True:
...
def train_resnet(
    ...
    *,
    ...
    best_f1 = -1.0
    stale = 0
    history = {"best_val_f1_macro": 0.0, "epochs_run": 0}
    ...
    history["best_val_f1_macro"] = best_f1 if early_stop else float("nan")
